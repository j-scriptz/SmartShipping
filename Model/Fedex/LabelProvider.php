<?php
/**
 * Jscriptz SmartShipping - FedEx Label Provider
 *
 * Implements LabelProviderInterface for FedEx carrier.
 * Ties together the FedEx clients for label creation and tracking.
 */
...
use Jscriptz\SmartShipping\Model\Fedex\Client\LabelClient;
use Jscriptz\SmartShipping\Model\Fedex\Request\LabelRequestBuilder;
use Jscriptz\SmartShipping\Model\Fedex\Response\LabelResponseParser;
...
class LabelProvider implements LabelProviderInterface
{
    ...
    private const SUPPORTED_FORMATS = ['PDF', 'PNG', 'ZPL', 'EPL'];
    ...
    public function createLabel(OrderInterface $order, array $packageData = []): ShipmentLabelInterface
    {
        ...
        if (!$this->isAvailable($storeId)) {
            throw new LocalizedException(
                __('FedEx label generation is not available. Please check configuration.')
            );
        }
        ...
        try {
            ...
            if ($this->config->isDebugEnabled($storeId)) {
                $this->logger->debug('[FEDEXv3 LabelProvider] Creating label for order', [
            ...
            if ($this->config->isDebugEnabled($storeId)) {
                $this->logger->debug('[FEDEXv3 LabelProvider] Label created successfully', [
            ...
            $this->logger->error('[FEDEXv3 LabelProvider] Label creation failed', [
                'order_id' => $order->getEntityId(),
                'error' => $e->getMessage(),
            ]);
            throw $e;
        } catch (\Exception $e) {
            $this->logger->error('[FEDEXv3 LabelProvider] Unexpected error', [
                'order_id' => $order->getEntityId(),
                'error' => $e->getMessage(),
                'exception' => $e,
            ]);
            throw new LocalizedException(
                __('Failed to create FedEx label: %1', $e->getMessage()),
                $e
            );
        }
    }
    ...
    {
        // TODO: Implement FedEx Track API client
        // Endpoint: POST /track/v1/trackingnumbers
        throw new LocalizedException(
            __('FedEx tracking API is not yet implemented.')
        );
    }
    ...
    public function voidLabel(string $trackingNumber, ?int $storeId = null): bool
    {
        try {
            if ($this->config->isDebugEnabled($storeId)) {
                $this->logger->debug('[FEDEXv3 LabelProvider] Voiding label', [
            ...
            $this->logger->error('[FEDEXv3 LabelProvider] Label void failed', [
                'tracking_number' => $trackingNumber,
                'error' => $e->getMessage(),
            ]);
            throw $e;
