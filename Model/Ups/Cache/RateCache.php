<?php
/**
 * Jscriptz SmartShipping - UPS Rate Caching with Fallback
 */
declare(strict_types=1);

namespace Jscriptz\SmartShipping\Model\Ups\Cache;
...
class RateCache
{
    private const CACHE_PREFIX = 'jscriptz_upsv3_rates_';
    private const CACHE_TAG = 'JSCRIPTZ_UPSV3_RATES';
        ...
        try {
            ...
            if ($age <= $ttl) {
                if ($this->config->isDebugEnabled($storeId)) {
                    $this->logger->debug('[UPSv3 Cache] Cache hit (age: ' . $age . 's)');
                }
            ...
            if ($this->config->isDebugEnabled($storeId)) {
                $this->logger->debug('[UPSv3 Cache] Cache expired (age: ' . $age . 's), available for fallback');
            }
            return null;

        } catch (\Exception $e) {
            $this->logger->warning('[UPSv3 Cache] Failed to load cache: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function loadExpired(string $cacheKey, ?int $storeId = null, bool $includeTransitTimes = false): ?array
    {
        ...
        try {
            ...
            if ($rates) {
                $age = time() - ($data['timestamp'] ?? 0);
                if ($this->config->isDebugEnabled($storeId)) {
                    $this->logger->info('[UPSv3 Cache] Using expired cache for fallback (age: ' . $age . 's)');
                }
            ...
            $this->logger->warning('[UPSv3 Cache] Failed to load expired cache: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function save(string $cacheKey, array $rates, ?int $storeId = null, array $transitTimes = []): void
    {
        try {
            ...
            if ($this->config->isDebugEnabled($storeId)) {
                ...
                $this->logger->debug("[UPSv3 Cache] Cached " . count($rates) . " rates, {$transitCount} transit times (TTL: {$ttl}s)");
            }

        } catch (\Exception $e) {
            $this->logger->warning('[UPSv3 Cache] Failed to save cache: ' . $e->getMessage());
        }
    }
    ...
    public function invalidateAll(): void
    {
        ...
        $this->logger->info('[UPSv3 Cache] Invalidated all cached rates');
    }
